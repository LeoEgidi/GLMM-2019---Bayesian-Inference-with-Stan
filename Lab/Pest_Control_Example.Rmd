---
title: "GLMM - Bayesian inference with Stan. Lab part"
author: ""
date: "`r Sys.Date()`"
output:
  html_document:
...
Let's encode this probability model in a Stan program. As a first attempt, we do not explicitly model the time variation, but we consider the $n$-th complaint as an independent observation for a regression model, thus:
...
* Save it as `simple_poisson_regression.stan`  in the stan\_programs folder 
...
$\texttt{neg_binomial_2_log}(\text{ints} \, y, \text{reals} \, \eta, \text{reals} \, \phi)$. Like the `poisson_log` function, this negative binomial mass function
